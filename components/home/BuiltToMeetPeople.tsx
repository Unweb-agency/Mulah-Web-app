      ...
      <Container className="flex flex-col gap-20 md:gap-30 lg:gap-40 justify-center items-center py-10 md:py-16 lg:py-20">
            ...
            <div className="flex flex-col gap-4 mt-6">
              {TraditionalToolsList.map((traditionalToolsFeatures, index) => (
                <div className="flex flex-row items-center gap-5" key={index}>
                  ...
                  <span className="text-sm md:text-lg font-semibold text-neutral-500 leading-tight">
                    {" "}
                    {traditionalToolsFeatures}{" "}
                  </span>
                </div>
              ))}
            </div>
            ...
            <div className="flex flex-col gap-4 mt-6">
              {MulahFeaturesList.map((MulahFeaturesList, index) => (
                <div className="flex flex-row items-center gap-5" key={index}>
                  ...
                  <span className="text-base md:text-lg font-semibold text-neutral-800 leading-tight">
                    {" "}
                    {MulahFeaturesList}{" "}
                  </span>
                </div>
        ...
        <div className="max-w-180 w-full flex flex-col justify-center items-center text-center">
          ...
          <p className="text-gray-500 font-medium text-sm md:text-base mt-3.5 sm:mt-4 md:mt-4.5 lg:mt-5 leading-tight">
            Mülah isn’t here to fix people or prescribe how they should live.
            Instead, it creates space for reflection—revealing patterns and
            helping users recognize alignment (or misalignment) between their
            goals, emotions, and everyday decisions.
          </p>
          <p className="text-gray-500 font-medium text-sm md:text-base mt-3.5 sm:mt-4 md:mt-4.5 lg:mt-5 leading-tight">
            Whether someone is saving, spending, rebuilding, or rethinking their
            habits,{" "}
            <span className="font-bold text-[#2C2C2C] text-[15px] md:text-[17px]">
              {" "}
              Mülah adapts to them.{" "}
            </span>
            — meeting each moment with clarity, context, and understanding.
          </p>
        </div>
      </Container>
    </div>
